...
class Gr00tN1d7Config(PretrainedConfig):
    ...
    # Condition router: learn per-cross-attn-block mixtures over candidate VLM
    # layers instead of the hardcoded single tap (hidden_states[-1]).
    use_condition_router: bool = False
    # Indices into the backbone hidden_states tuple: 0 = embedding output,
    # i = output of LLM layer i. None => all of range(0, select_layer + 1).
    router_candidate_layers: list[int] | None = None
    # Softmax logit bias on the incumbent (deepest candidate) column at init;
    # with K=13, bias 4.0 puts ~82% of the mixture on the incumbent layer.
    router_init_bias: float = 4.0
