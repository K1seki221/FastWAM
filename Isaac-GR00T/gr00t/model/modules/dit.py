...
class AlternateVLDiT(DiT):
    ...
    def forward(
        ...
        # Condition-router support: encoder_hidden_states may be per-cross-block
        # routed conditions [B, N_cross, S, D] (4-D) instead of one shared
        # tensor [B, S, D] (3-D, stock behavior).
        per_block_conditions = encoder_hidden_states.dim() == 4
        ...
        for idx, block in enumerate(self.transformer_blocks):
            if idx % 2 == 1:
                ...
                if per_block_conditions:
                    curr_encoder_hidden_states = encoder_hidden_states[:, idx // 2]
                else:
                    curr_encoder_hidden_states = encoder_hidden_states

                hidden_states = block(
                    hidden_states,
                    attention_mask=None,
                    encoder_hidden_states=curr_encoder_hidden_states,
