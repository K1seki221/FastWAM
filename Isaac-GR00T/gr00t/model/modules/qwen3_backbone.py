...
class Qwen3Backbone(torch.nn.Module):
    def __init__(
        ...
        extra_hidden_layer_indices: list[int] | None = None,
    ):
        """
        ...
        # Condition-router taps: hidden_states indices to expose (0 = embedding
        # output, i = LLM layer i). hidden_states is materialized regardless
        # (output_hidden_states=True), so this costs no extra backbone compute.
        self.extra_hidden_layer_indices = extra_hidden_layer_indices
    ...
    def forward(self, vl_input: BatchFeature) -> BatchFeature:
        ...
        if self.extra_hidden_layer_indices is not None:
            # [B, K, S, hidden_size] — candidate pool for the condition router.
            data["backbone_features_all"] = torch.stack(
                [hidden_states[i] for i in self.extra_hidden_layer_indices], dim=1
            )
