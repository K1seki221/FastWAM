...
class Gr00tN1d7Pipeline(ModelPipeline):
    ...
    def _create_model(self):
        ...
        if self.config.training.start_from_checkpoint is not None and not skip_weight_loading:
            ...
            # Condition-router params are absent from stock checkpoints:
            # identity-init them (incumbent norm <- trained vlln; logits bias
            # already set in the module ctor) instead of failing the load.
            router_missing = [k for k in missing_keys if "condition_router" in k]
            if router_missing:
                model.action_head.init_condition_router_from_vlln()
                logging.info(
                    f"condition_router params not in checkpoint - identity-initialized "
            ...
            other_missing = [
                k for k in missing_keys if "mask_token" not in k and "condition_router" not in k
            ]
            errors = []
            if other_missing:
