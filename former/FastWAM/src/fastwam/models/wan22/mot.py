...
class MoT(nn.Module):
    ...
    def forward_video_tokens(
        ...
        video_context_payload: Optional[dict],
        video_attention_mask: torch.Tensor,
        capture_layer_numbers: Optional[Sequence[int]] = None,
    ) -> torch.Tensor | tuple[torch.Tensor, dict[int, torch.Tensor]]:
        """Run the video expert and optionally capture one-based block outputs."""
        if "video" not in self.mixtures:
            raise ValueError("MoT requires `video` expert for `forward_video_tokens`.")
        if video_attention_mask.ndim != 2:
            raise ValueError(
                f"`video_attention_mask` must be 2D [S,S], got {tuple(video_attention_mask.shape)}."
            )
        if video_attention_mask.shape != (video_tokens.shape[1], video_tokens.shape[1]):
            raise ValueError(
                "Video attention mask/token mismatch: "
                f"mask={tuple(video_attention_mask.shape)}, tokens={video_tokens.shape[1]}."
            )

        capture_layers = (
            tuple(int(layer_number) for layer_number in capture_layer_numbers)
            if capture_layer_numbers is not None
            else ()
        )
        if len(set(capture_layers)) != len(capture_layers):
            raise ValueError(
                f"`capture_layer_numbers` must contain unique layers, got {capture_layers}."
            )
        invalid_layers = [
            layer_number
            for layer_number in capture_layers
            if layer_number < 1 or layer_number > self.num_layers
        ]
        if invalid_layers:
            raise ValueError(
                "`capture_layer_numbers` uses one-based VideoDiT layers in "
                f"[1,{self.num_layers}], got invalid layers {invalid_layers}."
            )
        capture_layer_set = set(capture_layers)
        ...
        for layer_idx in range(self.num_layers):
            ...
            attended = self._mixed_attention(
                q_cat=q,
                k_cat=k,
                v_cat=v,
                attention_mask=video_attention_mask,
            ...
            x = self._apply_post_with_optional_checkpoint(
                ...
                gate_mlp=gate_mlp,
                use_gradient_checkpointing=use_gradient_checkpointing,
                mixed_slice=attended,
                context_payload=video_context_payload,
            )
            layer_number = layer_idx + 1
            if layer_number in capture_layer_set:
                captured_tokens[layer_number] = x

        if capture_layer_numbers is not None:
            return x, captured_tokens
        return x
