...
def create_fastwam(
    ...
    vip_config = loss.get("vip", {})
    if isinstance(vip_config, DictConfig):
        vip_config = OmegaConf.to_container(vip_config, resolve=True)
    if vip_config is None:
        vip_config = {}
    if not isinstance(vip_config, dict):
        raise ValueError(f"`loss.vip` must be dict-like, got {type(vip_config)}")
    ...
    return FastWAM.from_wan22_pretrained(
        ...
        loss_lambda_vip=float(loss.get("lambda_vip", 0.0)),
        vip_gamma=float(vip_config.get("gamma", 0.98)),
        vip_num_negatives=int(vip_config.get("num_negatives", 0)),
        vip_same_instruction_negatives=bool(
            vip_config.get("same_instruction_negatives", False)
        ),
        vip_representation_mode=str(
            vip_config.get("representation_mode", "last_layer")
        ),
        vip_feature_dim=int(vip_config.get("feature_dim", 256)),
        vip_pool_hidden_dim=int(vip_config.get("pool_hidden_dim", 64)),
        vip_l1_weight=float(vip_config.get("l1_weight", 0.0)),
        vip_l2_weight=float(vip_config.get("l2_weight", 0.0)),
    )


def create_fastwam_future_cache(
    model_id: str,
    tokenizer_model_id: str,
    video_dit_config,
    tokenizer_max_len: int = 512,
    ...
    from .models.wan22.fastwam_future_cache import FastWAMFutureCache
    ...
    if isinstance(action_scheduler, DictConfig):
        action_scheduler = OmegaConf.to_container(action_scheduler, resolve=True)
    if action_scheduler is None:
        raise ValueError("`action_scheduler` is required for FastWAMFutureCache.")
    if not isinstance(action_scheduler, dict):
        raise ValueError(f"`action_scheduler` must be dict-like, got {type(action_scheduler)}")
    required_action_scheduler_keys = {"train_shift", "infer_shift", "num_train_timesteps"}
    missing_keys = required_action_scheduler_keys - set(action_scheduler.keys())
    ...
    return FastWAMFutureCache.from_wan22_pretrained(
        device=device,
        torch_dtype=model_dtype,
        model_id=model_id,
        tokenizer_model_id=tokenizer_model_id,
...
def create_fastwam_joint_bid(
    ...
    from .models.wan22.fastwam_joint_bid import FastWAMJointBid
    ...
    if isinstance(action_scheduler, DictConfig):
        action_scheduler = OmegaConf.to_container(action_scheduler, resolve=True)
    if action_scheduler is None:
        raise ValueError("`action_scheduler` is required for FastWAMJointBid.")
    if not isinstance(action_scheduler, dict):
        raise ValueError(f"`action_scheduler` must be dict-like, got {type(action_scheduler)}")
    required_action_scheduler_keys = {"train_shift", "infer_shift", "num_train_timesteps"}
    missing_keys = required_action_scheduler_keys - set(action_scheduler.keys())
    ...
    return FastWAMJointBid.from_wan22_pretrained(
        device=device,
        torch_dtype=model_dtype,
        model_id=model_id,
        tokenizer_model_id=tokenizer_model_id,
...
def run_training(cfg: DictConfig):
    ...
    trainer.train()
    # Do not rely only on the interpreter's atexit hook: explicitly flush and
    # close W&B after the final checkpoint has been written.
