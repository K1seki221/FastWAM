...
class Wan22Trainer:
    def __init__(self, model, train_dataset, val_dataset=None, *, cfg: DictConfig):
        ...
        vip_pooler = getattr(self.model, "vip_pooler", None)
        if vip_pooler is not None:
            trainable_params.extend(list(vip_pooler.parameters()))
    ...
    def _finish_wandb(self):
        ...
        try:
            ...
            # Training artifacts are already local and must not be reported as
            # failed only because the remote tracking service cannot flush.
            logger.exception(
                "W&B finish failed; the local run data remains under %s/wandb "
                "and can be uploaded later with `wandb sync`.",
                self.output_dir,
            )
    ...
    def _apply_dit_only_train_mode(model):
        ...
        vip_pooler = getattr(model, "vip_pooler", None)
        if vip_pooler is not None:
            vip_pooler.train()
            vip_pooler.requires_grad_(True)
    ...
    def _to_batched_eval_sample(sample):
        ...
        if video_proprio is not None:
            if not isinstance(video_proprio, torch.Tensor):
                raise TypeError(
                    "`sample['video_proprio']` must be a torch.Tensor, "
                    f"got {type(video_proprio)}"
                )
            if video_proprio.ndim == 2:
                video_proprio = video_proprio.unsqueeze(0)
            if video_proprio.ndim != 3:
                raise ValueError(
                    "`sample['video_proprio']` must be 3D [B,T_video,d], "
                    f"got shape {tuple(video_proprio.shape)}"
                )
            if video_proprio.shape[:2] != video.shape[:1] + (num_video_frames,):
                raise ValueError(
                    "`sample['video_proprio']` must align with the eval video: "
                    f"got {tuple(video_proprio.shape[:2])}, "
                    f"expected {(video.shape[0], num_video_frames)}"
                )
    ...
    def evaluate(self):
        ...
        # 2. inference and video saving. Action-only variants intentionally do
        # not run the expensive video denoising/decoding path during eval.
        supports_video_inference = bool(getattr(model, "supports_video_inference", True))
        if supports_video_inference:
            infer_kwargs = {
            ...
            if "num_video_frames" in inspect.signature(model.infer_action).parameters:
                action_infer_kwargs["num_video_frames"] = num_frames
            if sample["context"] is not None:
                ...
                action_infer_kwargs["prompt"] = prompt
        ...
        psnr_rollout_vs_gt = -1.0
        ssim_rollout_vs_gt = -1.0
        ...
        psnr_rollout_vs_decode = -1.0
        ssim_rollout_vs_decode = -1.0
        stitched_parts = [vae_video_tensor, gt_video_tensor]
        if pred_video_tensor is not None:
            ...
            stitched_parts.insert(0, pred_video_tensor)

        stitched_video_tensor = torch.cat(stitched_parts, dim=2).contiguous()
        stitched_frames = []
