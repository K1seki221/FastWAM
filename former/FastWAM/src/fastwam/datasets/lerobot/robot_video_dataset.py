...
class RobotVideoDataset(torch.utils.data.Dataset):
    ...
    def _get(self, idx):
        ...
        full_proprio = sample["proprio"]
        if full_proprio.ndim != 2 or full_proprio.shape[0] != self.num_frames:
            raise ValueError(
                "`sample['proprio']` must align with the full observation sequence: "
                f"expected [{self.num_frames}, D], got {tuple(full_proprio.shape)}."
            )
        # Preserve the state aligned with every temporally downsampled video
        # frame, including the final frame at index ``num_frames - 1``. The
        # action-aligned proprio below intentionally excludes that final state.
        video_proprio = full_proprio[self.video_sample_indices]
        proprio = full_proprio[:-1, :] # [T-1, state_dim], to align with action
        if video_proprio.shape[0] != video.shape[1]:
            raise ValueError(
                "Video/proprio temporal alignment mismatch after downsampling: "
                f"video frames={video.shape[1]}, proprio frames={video_proprio.shape[0]}."
            )
        if video.shape[1] <= 1:
