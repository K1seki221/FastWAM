...
run_libero_eval() {
    ...
    require_command() {
        ...
        if ! command -v "$command_name" >/dev/null 2>&1; then
            echo "Error: required command '$command_name' was not found. Please install tmux before running LIBERO parallel evaluation."
            exit 127
        fi
    }
    ...
    SESSION_NAME="${TMUX_SESSION_NAME:-libero_${RUN_ID}}"
    ...
    cleanup_tmux_session() {
        local exit_code=$?
        trap - EXIT INT TERM
        if [ "$SESSION_CREATED" -eq 1 ] && tmux has-session -t "$SESSION_NAME" 2>/dev/null; then
            echo "Stopping tmux session '$SESSION_NAME' and its evaluation workers..."
    ...
    print_task_log_excerpt() {
        local log_file=$1
        local tail_lines=${FAILURE_LOG_TAIL_LINES:-200}
        echo "----- worker failure log: $log_file (last $tail_lines lines) -----"
        if [ -s "$log_file" ]; then
            tail -n "$tail_lines" "$log_file" 2>&1 || true
        elif [ -e "$log_file" ]; then
            echo "[worker log exists but is empty; the worker may have been terminated before Python started]"
        else
            echo "[worker log does not exist; the tmux worker may not have started]"
        fi
        echo "----- end worker failure log -----"
    }
    ...
    # A unique session prevents one evaluation job from killing another.
    ...
    launch_task_on_pane() {
        ...
        tmux send-keys -t "$SESSION_NAME:$pane_info" "cd $ROOT_DIR && export EXP_NAME=$EXP_NAME && \
            STATUS_FILE='$status_file' LOG_FILE='$log_file' RESULT_FILE='$result_file' && \
            echo '[worker] started suite=$suite task_id=$task_id gpu=$gpu_id at '\"\$(date --iso-8601=seconds)\" > \"\$LOG_FILE\" && \
            echo '[worker] python=$PYTHON_EXECUTABLE' >> \"\$LOG_FILE\" && \
            echo '[worker] task=$CONFIG checkpoint=$CKPT' >> \"\$LOG_FILE\" && \
            CUDA_VISIBLE_DEVICES=$gpu_id MUJOCO_EGL_DEVICE_ID=$gpu_id \"$PYTHON_EXECUTABLE\" experiments/libero/eval_libero_single.py \
    ...
    cleanup_completed_tasks() {
        ...
        while IFS=: read -r task_info gpu_id; do
            ...
            # Only release a GPU after the worker shell has observed Python exit.
            # A result JSON is written before interpreter teardown, while the model
            # and EGL context may still occupy GPU memory.
            if [ -f "$status_file" ]; then
                ...
                if [ "$status" = "FAILED" ]; then
                    ...
                    print_task_log_excerpt "${status_log:-$TASK_LOG_DIR/${suite}_task${task_id}_gpu${gpu_id}.log}"
                    rm -f "$status_file"
                    continue
                fi
