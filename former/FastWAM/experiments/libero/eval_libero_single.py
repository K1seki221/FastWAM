...
if (
    len(cuda_visible_devices) == 1
    and os.environ.get("MUJOCO_EGL_DEVICE_ID") == cuda_visible_devices[0]
):
...
class ActionGenerationTimer:
    def __init__(self, *, enabled: bool, warmup_calls: int, device: str):
        if warmup_calls < 0:
            raise ValueError(f"EVALUATION.timing_warmup_calls must be non-negative, got {warmup_calls}")
        self.enabled = bool(enabled)
        self.warmup_calls = int(warmup_calls)
        self.device = str(device)
        self.cuda_synchronized = self.device.startswith("cuda") and torch.cuda.is_available()
        self.durations_seconds: list[float] = []
        self.baseline_allocated_bytes: list[int] = []
        self.peak_allocated_bytes: list[int] = []
        self.peak_reserved_bytes: list[int] = []
    ...
    def measure(self):
        ...
        if self.cuda_synchronized:
            ...
            baseline_allocated = torch.cuda.memory_allocated(device=self.device)
        ...
        try:
            ...
            self.durations_seconds.append(elapsed)
            if self.cuda_synchronized:
                self.baseline_allocated_bytes.append(baseline_allocated)
                self.peak_allocated_bytes.append(torch.cuda.max_memory_allocated(device=self.device))
                self.peak_reserved_bytes.append(torch.cuda.max_memory_reserved(device=self.device))
            call_idx = len(self.durations_seconds)
            phase = "warmup" if call_idx <= self.warmup_calls else "measured"
            logging.info(
                "Action generation timing: call=%d phase=%s duration=%.6f seconds",
                call_idx,
                phase,
                elapsed,
            )
    ...
    def summary(self) -> dict[str, Any]:
        skipped = min(self.warmup_calls, len(self.durations_seconds))
        measured = np.asarray(self.durations_seconds[skipped:], dtype=np.float64)
        result: dict[str, Any] = {
            "enabled": self.enabled,
            "scope": "observation preprocessing through executable action postprocessing",
            "cuda_synchronized": self.cuda_synchronized,
            "memory_scope": "per-call PyTorch CUDA allocator peaks including static model allocation",
            "total_calls": len(self.durations_seconds),
            "warmup_calls_requested": self.warmup_calls,
            "warmup_calls_skipped": skipped,
            "measured_calls": int(measured.size),
        }
        if measured.size == 0:
            result.update(
                {
                    ...
                    "min_seconds": None,
                    "max_seconds": None,
                    "mean_peak_allocated_gib": None,
                    "max_peak_allocated_gib": None,
                    "mean_peak_reserved_gib": None,
                    "max_peak_reserved_gib": None,
                    "mean_incremental_peak_allocated_gib": None,
                    "max_incremental_peak_allocated_gib": None,
                }
            )
            return result
        ...
        result.update(
            {
                ...
                "std_seconds": float(measured.std(ddof=0)),
                "median_seconds": float(np.median(measured)),
                "p95_seconds": float(np.percentile(measured, 95)),
                "min_seconds": float(measured.min()),
                "max_seconds": float(measured.max()),
            }
        )
        if self.cuda_synchronized:
            gib = float(1024**3)
            baseline = np.asarray(self.baseline_allocated_bytes[skipped:], dtype=np.float64)
            peak_allocated = np.asarray(self.peak_allocated_bytes[skipped:], dtype=np.float64)
            peak_reserved = np.asarray(self.peak_reserved_bytes[skipped:], dtype=np.float64)
            incremental_peak = peak_allocated - baseline
            result.update(
                {
                    "mean_peak_allocated_gib": float(peak_allocated.mean() / gib),
                    "max_peak_allocated_gib": float(peak_allocated.max() / gib),
                    "mean_peak_reserved_gib": float(peak_reserved.mean() / gib),
                    "max_peak_reserved_gib": float(peak_reserved.max() / gib),
                    "mean_incremental_peak_allocated_gib": float(incremental_peak.mean() / gib),
                    "max_incremental_peak_allocated_gib": float(incremental_peak.max() / gib),
                }
            )
        else:
            result.update(
...
def _validate_inference_mode_cfg(cfg: DictConfig) -> None:
    joint_inference = bool(cfg.EVALUATION.get("joint_inference", False))
    skip_vae_decode = bool(cfg.EVALUATION.get("skip_vae_decode", False))
    visualize_future_video = bool(cfg.EVALUATION.get("visualize_future_video", False))
    if skip_vae_decode and not joint_inference:
        raise ValueError("EVALUATION.skip_vae_decode=true requires EVALUATION.joint_inference=true.")
    if skip_vae_decode and visualize_future_video:
        raise ValueError(
            "EVALUATION.skip_vae_decode=true is incompatible with "
...
def _predict_action_chunk(
    ...
    *,
    ...
    visualize_future_video = bool(cfg.EVALUATION.get("visualize_future_video", False))
    joint_inference = bool(cfg.EVALUATION.get("joint_inference", False)) or visualize_future_video
    skip_vae_decode = bool(cfg.EVALUATION.get("skip_vae_decode", False))
    ...
    with torch.no_grad():
        if joint_inference and not skip_vae_decode:
...
def run_single_episode(
    ...
    *,
    ...
    input_h: int,
    model_device: str,
    action_generation_timer: ActionGenerationTimer,
    ...
    timing_max_calls_cfg = cfg.EVALUATION.get("timing_max_calls", None)
    timing_max_calls = None if timing_max_calls_cfg is None else int(timing_max_calls_cfg)
    if timing_max_calls is not None and timing_max_calls <= 0:
        raise ValueError(
            f"EVALUATION.timing_max_calls must be positive when set, got {timing_max_calls}"
        )
    ...
    while t < max_steps + num_steps_wait:
        ...
        if len(pending_actions) == 0:
            with action_generation_timer.measure():
            ...
            if (
                action_generation_timer.enabled
                and timing_max_calls is not None
                and action_generation_timer.total_calls >= timing_max_calls
            ):
                logging.info(
                    "Stopping benchmark episode after %d action-generation calls.",
                    action_generation_timer.total_calls,
                )
                break
        else:
            imgs = get_libero_image(obs)
    ...
    try:
        for trial_idx in range(int(cfg.EVALUATION.num_trials)):
            success, replay_images, predicted_future_video_clips, episode_mean_psnr = run_single_episode(
                ...
                input_h=input_h,
                model_device=model_device,
                action_generation_timer=action_generation_timer,
            )
            if success:
                results["successes"] += 1
        ...
        try:
            ...
            logging.exception("Failed to close the LIBERO environment cleanly.")
        finally:
            # Trigger the MuJoCo / EGL destructors while EGL is still initialized,
            # instead of leaving them to interpreter shutdown.
            env = None
            gc.collect()
...
def eval_single_process(cfg: DictConfig):
    ...
    _validate_inference_mode_cfg(cfg)
    ...
    action_generation_timer = ActionGenerationTimer(
        enabled=bool(cfg.EVALUATION.get("timing_enabled", False)),
        warmup_calls=int(cfg.EVALUATION.get("timing_warmup_calls", 3)),
        device=model_device,
    )
    ...
    results["action_generation_timing"] = action_generation_timer.summary()
    ...
    timing_summary = results["action_generation_timing"]
    if timing_summary["enabled"]:
        print(
            "Action generation timing: "
            f"mean={timing_summary['mean_seconds']!s} seconds, "
            f"measured={timing_summary['measured_calls']}, "
            f"warmup_skipped={timing_summary['warmup_calls_skipped']}, "
            f"max_peak_allocated={timing_summary['max_peak_allocated_gib']!s} GiB, "
            f"max_peak_reserved={timing_summary['max_peak_reserved_gib']!s} GiB"
        )
