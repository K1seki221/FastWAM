...
echo "[launch] nproc_per_node=${NPROC_PER_NODE} num_machines=${NUM_MACHINES} machine_rank=${MACHINE_RANK} run_id=${RUN_ID} run_name=${RUN_NAME:-<none>} run_dir_id=${RUN_DIR_ID}"
...
# Replace this launcher process with Accelerate. Besides preserving signal and
# exit-code behavior, this prevents Bash from reading the launcher file again
# after a many-hour training command returns (the workspace file may have been
# updated while the job was running).
exec "${ACCELERATE_CMD[@]}" launch \
  --config_file scripts/accelerate_configs/accelerate_zero1_ds.yaml \
  --num_processes "${NPROC_PER_NODE}" \
  scripts/train.py \
  "output_dir=/dataset_rc/zhangxp7@xiaopeng.com/projects/fastwam/runs/${TASK_BASENAME}/${RUN_DIR_ID}" \
