...
# Replace this launcher process with Accelerate so a live workspace update
# cannot affect the shell after the training command returns.
exec accelerate launch \
  --config_file scripts/accelerate_configs/accelerate_zero2_ds.yaml \
  --num_processes "${NPROC_PER_NODE}" \
  scripts/train.py \
  "output_dir=./runs/${TASK_BASENAME}/${RUN_DIR_ID}" \
