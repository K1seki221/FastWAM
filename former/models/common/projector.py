...
class PlainProjector(nn.Module):
    """
    Linear wrapper for fsdp auto warp purpose
    """

    def __init__(self, input_dim: int, output_dim: int, bias: bool = True) -> None:
        super().__init__()
        self.projector = nn.Linear(input_dim, output_dim, bias=bias)
        with torch.no_grad():
            self.projector.weight.copy_(0.02 * torch.randn_like(self.projector.weight))
            if self.projector.bias is not None:
                self.projector.bias.zero_()

    def forward(self, input_tensor: torch.Tensor) -> torch.Tensor:
        return self.projector(input_tensor)
    ...
    def _init_weights(self):
        for m in self.projector:
            if isinstance(m, nn.Linear):
                with torch.no_grad():
                    m.weight.copy_(0.02 * torch.randn_like(m.weight))
